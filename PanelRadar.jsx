...
// ---------------------------------------------------------------------------
// ANCLA — Módulo de Propósito ("Para quién")
//
// Respaldo directo en el libro: ejercicio "Las Tres Preguntas del Propósito
// Real" (Principio 5) — en vez de metas abstractas ("comprar casa"), el
// usuario define para quién está reconstruyendo sus finanzas, con nombre
// concreto. Cada meta de ahorro se ancla a una persona o motivo, no solo
// a un monto.
//
// Esta versión reutiliza la capa de autenticación mock del módulo anterior
// y agrega una 6ª pestaña "Propósito" a la navegación.
// ---------------------------------------------------------------------------
...
const seedGoals = [
  { id: 1, person: "Mis hijos", why: "Que me vean levantarme, no solo caer.", target: 20000, saved: 6500 },
];

// =============================================================================
// AUTENTICACIÓN (mock, igual que el módulo anterior)
// =============================================================================
    ...
    <div className="flex items-center gap-2 px-4 py-3 rounded-lg mb-4" style={{ background: dark ? palette.inkSoft : "#FFFFFF", border: `1px solid ${dark ? palette.inkLine : palette.paperLine}` }}>
      ...
      {isPassword && (
        <button type="button" onClick={() => setShow(!show)}>
          {show ? <EyeOff size={16} color={palette.ash} /> : <Eye size={16} color={palette.ash} />}
        </button>
      )}
    </div>
  );
...
function WaitScreen({ onBack }) {
  return (
    <div className="min-h-full flex flex-col items-center justify-center p-8 text-center" style={{ background: palette.ink }}>
      ...
      <p className="text-sm max-w-xs leading-relaxed mb-8" style={{ ...sans, color: palette.ash }}>Vuelve cuando estés listo. Esto va a seguir aquí, sin juzgarte por el tiempo que tomes.</p>
  ...
  return (
    <div className="min-h-full flex flex-col justify-between p-8" style={{ background: palette.ink }}>
      ...
      <div className="flex-1 flex flex-col justify-center max-w-md mx-auto text-center">
        ...
        <p className="text-base leading-relaxed mb-2" style={{ ...sans, color: "#C7CAD4" }}>No vamos a pedirte que arregles nada todavía. Solo que mires tus números de frente.</p>
    ...
    <div className="min-h-full flex flex-col p-6" style={{ background: palette.ink }}>
      <div className="max-w-md mx-auto w-full flex-1 flex flex-col">
        <div className="mb-6">
          ...
          <p className="text-xs mt-2" style={{ ...sans, color: palette.ash }}>{debts.length === 0 ? "Empecemos por lo primero que te venga a la mente." : `${debts.length} registrados — puedes seguir o pausar.`}</p>
        </div>
        <PaperCard>
          {stage === "who" && (
            <>
...
// =============================================================================
// PANEL / RADAR / DIARIO / REVISIÓN (igual que antes)
// =============================================================================
function TruthPanel({ debts, streak, onGoRadar, name }) {
  const total = debts.reduce((s, d) => s + d.remaining, 0);
  ...
  return (
    <div className="p-6">
      ...
      <h2 className="text-2xl mb-6" style={{ ...serif, color: palette.paperText }}>Hoy, {name}, esto es lo que importa.</h2>
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center p-6 z-50" style={{ background: "rgba(20,23,31,0.7)" }}>
      <div className="rounded-2xl p-8 max-w-xs w-full text-center" style={{ background: palette.paper, border: `1px solid ${palette.paperLine}` }}>
        ...
        <p className="text-sm leading-relaxed mb-6" style={{ ...sans, color: palette.ashPaper }}>No necesitas más celebración que esta. Lo que pagabas aquí ahora empuja tu siguiente batalla.</p>
...
function DebtRadar({ debts, onPay }) {
  ...
  return (
    <div className="p-6">
      ...
      <p className="text-sm leading-relaxed mb-5" style={{ ...sans, color: palette.ashPaper }}>Cada abono va primero a tu foco actual. Al liquidarla, ese pago se suma a la siguiente.</p>
...
function DiaryScreen({ entries, onAddEntry }) {
  ...
  return (
    <div className="p-6">
      ...
      <p className="text-sm leading-relaxed mb-5" style={{ ...sans, color: palette.ashPaper }}>Nombrar con precisión es el primer acto de sanidad.</p>
    ...
    <div className="p-6">
      ...
      <div className="rounded-xl p-4 mb-6" style={{ background: palette.pineSoft }}>
        <p className="text-xs leading-relaxed" style={{ ...sans, color: palette.pineDeep }}>
          Por ahora esto es una pantalla de prueba: tus datos no están en un servidor todavía.
        </p>
      </div>
      <GhostButton onClick={logout} style={{ width: "100%", color: palette.errorText, borderColor: palette.errorText }}>
...
// =============================================================================
// MÓDULO DE PROPÓSITO — "Para quién"
// =============================================================================
function AddGoalForm({ onAdd, onCancel }) {
  ...
  return (
    <PaperCard>
      ...
      <p className="text-xs mb-4" style={{ ...sans, color: "#6B6F5F" }}>
        No pongas un concepto ("vacaciones"). Pon un nombre — el rostro que te hace seguir cuando no tengas ganas.
      </p>
      <input
        autoFocus
        value={person}
        onChange={(e) => setPerson(e.target.value)}
        placeholder="Ej: Mis hijos, mi esposa, yo mismo..."
      ...
      <textarea
        ...
        placeholder="¿Qué quieres que vean en ti? (opcional)"
        rows={2}
...
function GoalCard({ goal, index, onContribute }) {
  ...
  return (
    <div className="rounded-xl p-5 mb-3" style={{ background: palette.paperCard, border: `1px solid ${palette.paperLine}` }}>
      ...
      {!complete && (
        !open ? (
          <button onClick={() => setOpen(true)} className="text-xs px-3 py-2 rounded-lg" style={{ ...sans, color: palette.pine, border: `1px solid ${palette.pine}` }}>
            Aportar
          </button>
        ) : (
          <div className="flex items-center gap-2">
...
function PurposeScreen({ goals, onAddGoal, onContribute }) {
  ...
  return (
    <div className="p-6">
      ...
      <p className="text-sm leading-relaxed mb-5" style={{ ...sans, color: palette.ashPaper }}>
        No para qué — para quién. Los rostros concretos que vienen a tu mente son tu propósito más honesto.
      </p>

      {goals.map((g, i) => (
        <GoalCard key={g.id} goal={g} index={i} onContribute={onContribute} />
      ))}
      ...
      {showForm ? (
        ...
        <button
          ...
          className="w-full rounded-xl p-4 flex items-center justify-center gap-2 text-sm transition-all duration-200"
      ...
      {goals.length === 0 && !showForm && (
        <p className="text-xs text-center mt-4" style={{ ...sans, color: palette.ashPaper }}>
          Aún no tienes ninguna meta con nombre. Empieza por una persona, no por un monto.
        </p>
      )}
    </div>
  );
...
// =============================================================================
// NAVEGACIÓN INFERIOR (ahora con 6 pestañas — se vuelve scroll horizontal si hace falta)
// =============================================================================
      ...
      {items.map((it) => {
        ...
        return (
          <button key={it.key} onClick={() => setTab(it.key)} className="flex flex-col items-center gap-1 px-1.5 py-1 flex-shrink-0">
            ...
            <span className="text-[8.5px]" style={{ ...sans, color: active ? palette.pine : palette.ash }}>{it.label}</span>
          </button>
        );
      })}
    </div>
      ...
      <div className="w-full mx-auto rounded-3xl overflow-hidden" style={{ maxWidth: 420, minHeight: 720, boxShadow: "0 30px 60px -20px rgba(0,0,0,0.5)" }}>
        ...
        {onboardPhase === "reveal" && (
          <RealityReveal
            ...
            onContinue={() => { setDebts(debts.length ? debts : initialDebts); setOnboardPhase("done"); }}
          />
        )}
      </div>
    );
  ...
  return (
    <div className="w-full mx-auto rounded-3xl overflow-hidden flex flex-col relative" style={{ maxWidth: 420, minHeight: 720, boxShadow: "0 30px 60px -20px rgba(0,0,0,0.5)", background: palette.paper }}>
      <div className="flex-1 overflow-y-auto">
        {tab === "home" && <TruthPanel debts={debts} streak={streak} onGoRadar={() => setTab("radar")} name={firstName} />}
        {tab === "radar" && <DebtRadar debts={debts} onPay={handlePay} />}
        {tab === "purpose" && <PurposeScreen goals={goals} onAddGoal={(g) => setGoals([...goals, g])} onContribute={handleContribute} />}
